...
async def inspect_service(
    *,
    ...
    ready = (
        await llama_cpp._probe_ready(_endpoint(host_port))
...
async def stop_service(
    *,
    ...
    if current.container_id is None:
        raise PersistentQwenServiceError(
            "Persistent Qwen inspection lacked a container ID.",
        )
    if current.container_running:
        stopped = await executor.execute(
            Command("docker", ("stop", current.container_id)),
        )
        if stopped.failed:
            raise PersistentQwenServiceError(
...
def _parse_inspection(output: bytes) -> _Inspection:
    ...
    if len(lines) != 1:
        raise PersistentQwenServiceError(
            "Docker inspection was not one state projection.",
        )
    parts = lines[0].split("|")
    if len(parts) != 5:
        raise PersistentQwenServiceError(
            "Docker inspection had an invalid service projection.",
        )
    ...
    if len(container_id) != _CONTAINER_ID_LENGTH or any(
        ...
        raise PersistentQwenServiceError(
            "Docker inspection returned an invalid container ID.",
        )
    if running.casefold() not in {"true", "false"}:
        raise PersistentQwenServiceError(
            "Docker inspection returned an invalid running state.",
        )
