...
from devtools.core.regex import search_regex
...
from devtools.resources.commands import Command, CommandExecutor, CommandResult
...
_MANAGED_LABEL = "devtools.models.serving.managed"
_MANAGED_VALUE = "true"
...
def _advertises_n_cpu_ffn(help_text: str) -> bool:
    ...
    return search_regex(help_text, r"(?<!\S)--n-cpu-ffn(?=\s|=|,|$)") is not None
...
def _inspect_command(container_id: str) -> Command:
    return Command(
        ...
        (
            "inspect",
            "--format",
            '{{.State.Running}}|{{index .Config.Labels "devtools.models.serving.managed"}}',
            container_id,
        ),
    )
