...
from devtools.core.regex import replace_regex
...
_SCHEMA_VERSION = 5
_UNSAFE_FILENAME = r"[^a-zA-Z0-9._-]+"
...
def save_benchmark_result(
    ...
    destination = ResolvedPath(output_root.value / filename)
...
def load_benchmark_result(path: ResolvedPath) -> ModelBenchmarkResult:
    ...
    try:
        source = cast("TextFile", read(path, file_format=FileFormat.TEXT))
...
def _from_json(raw: object) -> ModelBenchmarkResult:
    ...
    return ModelBenchmarkResult(
        ...
        serving=VLLMBenchmarkServingSnapshot(
            ...
            cpu_offload_gb=(
                0.0 if schema_version == 1 else _number(serving["cpu_offload_gb"])
            ),
            offload_backend=(
                None
...
def _result_filename(result: ModelBenchmarkResult) -> str:
    ...
    name = replace_regex(result.case.name, _UNSAFE_FILENAME, "-").strip(".-")
