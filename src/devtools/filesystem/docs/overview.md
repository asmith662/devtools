...
`devtools.filesystem` provides immutable file models, representation codecs,
...
Filesystem consumes `ResolvedPath` from `devtools.paths`, generic regex
mechanics from `devtools.regex`, and callable conversion from
`devtools.conversion`. It does not depend on `devtools.system`.
