...
`devtools.core.regex` is a small reusable layer over Python's `re` module. It
...
`devtools.core.regex` depends only on Python standard-library regex and typing
facilities. It has no dependency on another `devtools` package.

Its primary production consumer is `devtools.resources.filesystem.models.TextFile`,
