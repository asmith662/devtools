...
`devtools.core.paths` owns path representation, parsing, absolute resolution, and a
...
facilities. It has no dependency on `devtools.core.system` or `devtools.resources.filesystem`.
Domains such as commands and filesystem consume `ResolvedPath` as their
absolute-path input value.

## Package boundaries

`devtools.core.system` is not needed for the current standard-library home and
temporary-directory mechanisms or the documented Downloads convention.

`devtools.core.paths` does not own reading, writing, format detection, file models,
filesystem traversal, mutation, or file metadata inspection; these belong to
`devtools.resources.filesystem` where applicable.
