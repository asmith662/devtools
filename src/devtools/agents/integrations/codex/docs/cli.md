...
`devtools.agents.integrations.codex` is a narrow adapter over the local Codex CLI, not a general
...
`ConversationMessage.content` is one direct argv value. No shell participates, so spaces,
