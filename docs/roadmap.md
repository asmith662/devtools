...
| `message` | Immutable textual communication values | Documented / Frozen | Complete |
...
provider, session, or runtime state.
...
The next architectural design frontier is `devtools.session`: a domain for
representing and organizing durable ordered interaction and execution history.
Its public API, models, and storage policy remain intentionally undesigned.
