...
from devtools.models.serving import vllm
from devtools.models.serving.errors import (
...
from devtools.resources.commands import Command, CommandNotFoundError, CommandResult
...
def test_start_builds_deterministic_owned_docker_launch(
    ...
    monkeypatch.setattr("devtools.models.serving.vllm._probe_model_ready", ready)
    config = _config(tmp_path)
    server = asyncio.run(
        VLLMServer.start(
            config=config,
    ...
    assert "devtools.models.serving.managed=true" in command.arguments
...
def test_start_rejects_multiline_container_id_before_readiness(
    ...
    monkeypatch.setattr(
        "devtools.models.serving.vllm._probe_model_ready",
        unexpected_probe,
    )
    executor = _Executor([])
    executor.execute = execute  # type: ignore[method-assign]
...
def test_readiness_timeout_stops_owned_container(
    ...
    monkeypatch.setattr("devtools.models.serving.vllm._probe_model_ready", not_ready)
...
def test_probe_model_ready_interprets_only_matching_model(
    ...
    monkeypatch.setattr("devtools.models.serving.vllm._http_get", get)
