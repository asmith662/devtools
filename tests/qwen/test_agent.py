# Copyright (c) 2026
"""Deterministic tests for the provider-local Qwen Agent adapter."""
...
from devtools.qwen import QwenAgent, QwenHttpError, QwenResponseError
from devtools.qwen import agent as qwen_agent
...
def _agent() -> QwenAgent:
    return QwenAgent(endpoint="http://127.0.0.1:8080", model="qwen-local")


@pytest.mark.parametrize("role", [MessageRole.USER, MessageRole.SYSTEM])
def test_agent_maps_existing_message_roles_to_non_streaming_llama_request(
    ...
    turn = asyncio.run(_agent().send(_message(role, "exact content")))
...
def test_agent_rejects_malformed_successful_response(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A malformed success payload cannot become a final AgentTurn."""
    ...
    with pytest.raises(QwenResponseError, match="nonempty choices"):
        asyncio.run(_agent().send(_message(MessageRole.USER)))


def test_agent_preserves_non_success_http_as_provider_local_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with pytest.raises(QwenHttpError, match="HTTP 400: bad model") as raised:
        asyncio.run(_agent().send(_message(MessageRole.USER)))
    assert raised.value.status == _HTTP_BAD_REQUEST


def test_agent_rejects_continuation_for_stateless_provider() -> None:
    """The first Qwen adapter does not silently ignore provider continuation."""
...
def test_agent_rejects_endpoint_shapes_outside_its_narrow_origin_contract(
    endpoint: str,
) -> None:
    """Provider adaptation accepts only the LlamaCppServer-style HTTP origin."""
    with pytest.raises(ValueError, match="endpoint"):
        QwenAgent(endpoint=endpoint, model="qwen-local")


def test_agent_rejects_blank_model_name() -> None:
    """The explicit served-model request field cannot be blank."""
    with pytest.raises(ValueError, match="model name"):
        QwenAgent(endpoint="http://127.0.0.1:8080", model=" ")


def test_agent_preserves_transport_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with pytest.raises(QwenTransportError, match="Could not communicate"):
        asyncio.run(_agent().send(_message(MessageRole.USER)))
...
def test_agent_rejects_invalid_http_or_success_payload_shapes(
    monkeypatch: pytest.MonkeyPatch,
    response: bytes,
    match: str,
) -> None:
    """Malformed framing and successful payloads never form AgentTurns."""
    ...
    with pytest.raises(QwenResponseError, match=match):
        asyncio.run(_agent().send(_message(MessageRole.USER)))
...
def test_agent_rejects_invalid_final_assistant_message_shapes(
    response: dict[str, object],
    match: str,
) -> None:
    """Only final textual assistant content is consumed from provider JSON."""
    with pytest.raises(QwenResponseError, match=match):
        qwen_agent._final_assistant_content(response)  # noqa: SLF001
...
def test_provider_error_detail_falls_back_when_error_shape_is_not_usable() -> None:
    ...
    assert qwen_agent._error_detail(b"not-json") == "not-json"  # noqa: SLF001
    assert qwen_agent._error_detail(b'{"error":{}}') == '{"error":{}}'  # noqa: SLF001
    assert qwen_agent._error_detail(b"[]") == "[]"  # noqa: SLF001
    assert qwen_agent._error_detail(b'{"error":[]}') == '{"error":[]}'  # noqa: SLF001


def test_provider_helpers_reject_defensive_invalid_origin_and_empty_headers() -> None:
    """Private transport helpers retain malformed states as Qwen-local failures."""
    with pytest.raises(QwenTransportError):
        asyncio.run(qwen_agent._post_chat_completion(urlsplit("http:///"), {}))  # noqa: SLF001

    async def read_empty_headers() -> None:
        with pytest.raises(QwenResponseError, match="status line"):
            await qwen_agent._read_http_response(_reader(b"\r\n\r\n"))  # noqa: SLF001

    asyncio.run(read_empty_headers())


def test_agent_reads_fragmented_content_length_response(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    turn = asyncio.run(_agent().send(_message(MessageRole.USER)))

    assert turn.message.content == "fragmented"
    assert deliveries[0].done()


def test_agent_reads_fragmented_chunked_response(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_agent_rejects_malformed_http_body_framing(
    monkeypatch: pytest.MonkeyPatch,
    response: bytes,
    match: str,
) -> None:
    """Malformed provider HTTP framing remains a Qwen-local response failure."""
...
async def _read_framed_response(
    ...
    *,
    ...
    reader.feed_data(response)
    reader.feed_eof()
    return await qwen_agent._read_http_response(reader)  # noqa: SLF001
...
def test_private_http_reader_rejects_invalid_header_or_framing(
    ...
    """Header and framing ambiguity stay inside Qwen's private transport."""
    with pytest.raises(QwenResponseError, match=match):
...
def test_private_http_reader_rejects_header_limit_overrun() -> None:
    ...
    with pytest.raises(QwenResponseError, match="headers exceeded"):
...
async def _read_http_line_with_limit() -> bytes:
    ...
    reader.feed_eof()
    return await qwen_agent._read_http_line(reader, "chunk size")  # noqa: SLF001
...
def test_private_http_reader_rejects_chunk_line_limit_overrun() -> None:
    ...
    with pytest.raises(QwenResponseError, match="chunk size exceeded"):
...
async def _read_incomplete_http_line() -> bytes:
    ...
    reader.feed_eof()
    return await qwen_agent._read_http_line(reader, "chunk trailer")  # noqa: SLF001
...
def test_private_http_reader_rejects_incomplete_chunk_line() -> None:
    ...
    with pytest.raises(QwenResponseError, match="ended during HTTP chunk trailer"):
