...
def test_parse_dot_path_converts_components_and_optional_suffix() -> None:
    ...
    assert parse_dot_path("devtools.paths.models") == Path(
        "devtools", "paths", "models",
    )
    assert parse_dot_path("devtools.paths.models", suffix="py") == Path(
        "devtools", "paths", "models.py",
    )
    assert parse_dot_path("devtools.paths.models", suffix=".py") == Path(
        "devtools", "paths", "models.py",
    )
...
def test_known_directory_helpers_return_conventional_absolute_paths(
    ...
    monkeypatch.setattr(
        "devtools.paths.resolution.tempfile.gettempdir",
