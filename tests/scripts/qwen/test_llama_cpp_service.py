...
_SERVICE_PATH = _SCRIPT_DIRECTORY / "llama_cpp_service.py"
_ID = "a" * 64
...
def test_profile_fingerprint_and_configuration_are_canonical(
    ...
    config = service._serving_config(
        ResolvedPath(tmp_path / profile.model.filename),
        8080,
    )
...
def test_malformed_port_is_conflict_without_lifecycle_authority(
    ...
    inspection = _result(
        Command("docker"),
        out=_inspection(service, running=False, port=port),
    )
    status = asyncio.run(
        service.inspect_service(
            executor=_Executor([inspection]),
...
def test_start_absent_acquires_semantic_artifact_launches_and_waits(
    ...
    assert (
        f"devtools.qwen.persistent-service={service._SERVICE_VALUE}" in launch.arguments
    )
    assert (
        f"devtools.qwen.profile-sha256={service.profile_fingerprint()}"
        in launch.arguments
    )
...
def test_start_handles_stale_profile_by_running_state(
    ...
    stale = _result(
        Command("docker"),
        out=_inspection(service, running=running, fingerprint="b" * 64),
    )
    if running:
        executor = _Executor([stale])
        with pytest.raises(
            service.PersistentQwenServiceError,
            match="stop it explicitly",
        ):
            asyncio.run(
                service.start_service(
                    cache_root=ResolvedPath(tmp_path),
...
def test_start_recreates_stopped_service_and_reports_port_conflict(
    ...
    assert (
        ...
        is service.PersistentQwenServiceState.READY
    )
...
def test_stop_is_safe_for_absent_owned_and_foreign_services(
    ...
    assert [command.arguments[0] for command in owned_executor.commands] == [
        "inspect",
        "stop",
        "rm",
    ]
...
def test_parser_and_helpers_validate_operational_inputs(service: ModuleType) -> None:
    ...
    parsed = service.parse_arguments(
        ["start", "--cache-root", "cache", "--port", "8123"],
    )
