...
import devtools.agents.integrations.codex
from devtools.agents.conversation.message import (
    ConversationMessage,
    ConversationMessageRole,
)
from devtools.agents.integrations.codex import (
    CodexAgent,
    CodexCommandError,
    CodexError,
    CodexOutputError,
)
from devtools.agents.integrations.codex.agent import _resolve_executable
...
_CODEX_AGENT_PLATFORM = "devtools.agents.integrations.codex.agent.sys.platform"
_CODEX_AGENT_WHICH = "devtools.agents.integrations.codex.agent.shutil.which"
...
def test_agent_builds_fresh_read_only_jsonl_command_and_maps_turn(
    ...
    interaction = _agent(executor, tmp_path)
    turn = asyncio.run(interaction.send(_user_message()))

    assert interaction.source == InteractionSource("codex")
    assert turn.message.content == "Codex reply"
    assert turn.message.role == ConversationMessageRole.ASSISTANT
...
@pytest.mark.parametrize(
    "role",
    [ConversationMessageRole.ASSISTANT, ConversationMessageRole.SYSTEM],
)
...
def test_agent_rejects_foreign_conversation_without_executing(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="owned by the codex"):
        asyncio.run(
            agent.send(
                _user_message(),
                conversation=ConversationRef(InteractionSource("qwen"), "foreign"),
            ),
        )
    assert executor.commands == []
...
def test_agent_rejects_unsuccessful_truncated_or_invalid_results(
    ...
    """Only an untruncated successful command can become an ModelResponse."""
...
def test_root_public_api_is_exact() -> None:
    ...
    assert devtools.agents.integrations.codex.__all__ == [
        "CodexAgent",
        "CodexCommandError",
    ...
    assert CodexError is devtools.agents.integrations.codex.CodexError


def test_agent_preserves_an_empty_prompt_as_one_argument(tmp_path: Path) -> None:
    """Direct argv transport does not discard a valid empty ConversationMessage content."""
    executor = RecordingExecutor([(0, _SUCCESS, False)])
    agent = _agent(executor, tmp_path)
