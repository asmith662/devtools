# Copyright (c) 2026
# ruff: noqa: E501
"""Tests for mutable retained Conversation state."""
...
def _message(
    ...
    *,
    ...
    return ConversationMessage(
        ...
        content=value,
        role=role,
        source=InteractionSource("test"),
    )


def _ref(source: str, value: str) -> ConversationRef:
    """Create one generic provider continuation reference."""
    return ConversationRef(InteractionSource(source), value)


def _session(*, conversations: object = ()) -> Conversation:
    """Create a deterministic Conversation with configurable reconstruction refs."""
    return Conversation(
        id=ConversationId.parse("12345678-1234-5678-1234-567812345678"),
...
def test_session_direct_reconstruction_preserves_supplied_state() -> None:
    ...
    identifier = ConversationId.parse("12345678-1234-5678-1234-567812345678")
    created_at = Timestamp(datetime(2026, 8, 11, 12, 0, tzinfo=UTC))
    ...
    assert session.conversation_for(InteractionSource("codex")) is ref
...
def test_session_public_lifecycle_properties_cannot_be_reassigned() -> None:
    ...
    with pytest.raises(AttributeError):
        session.id = ConversationId.new()  # type: ignore[misc]
    with pytest.raises(AttributeError):
...
def test_session_adds_all_message_roles_and_preserves_snapshots_and_duplicates() -> (
    ...
    """Conversation mutation composes immutable History without role restrictions."""
...
def test_session_constructor_materializes_supported_conversation_iterables(
    ...
    assert session.conversation_for(InteractionSource("codex")) == _ref(
        "codex",
        "thread-1",
    )
...
def test_session_rejects_duplicate_constructor_sources_by_value() -> None:
    ...
    duplicate_sources = (
        ConversationRef(InteractionSource("codex"), "thread-1"),
        ConversationRef(InteractionSource("codex"), "thread-2"),
    )
...
def test_session_conversations_are_live_read_only_source_keyed_state() -> None:
    """Read-only mapping exposure tracks controlled Conversation mutations."""
    session = _session()
    view = session.conversations
    codex = _ref("codex", "thread-1")
    qwen = _ref("qwen", "thread-2")

    assert session.conversation_for(InteractionSource("missing")) is None
    assert session.set_conversation(codex) is None  # type: ignore[func-returns-value]
    session.set_conversation(qwen)

    assert view[InteractionSource("codex")] is codex
    assert view[InteractionSource("qwen")] is qwen
    assert set(view) == {InteractionSource("codex"), InteractionSource("qwen")}
    with pytest.raises(TypeError):
        view[InteractionSource("local")] = _ref("local", "thread-3")  # type: ignore[index]
    with pytest.raises(TypeError):
        del view[InteractionSource("codex")]  # type: ignore[attr-defined]
...
def test_session_replaces_current_conversation_for_one_source() -> None:
    ...
    assert session.conversation_for(InteractionSource("codex")) is replacement
...
def test_sessions_use_object_identity_equality_and_are_unhashable() -> None:
    """Mutable Conversation entities separate Python equality from ConversationId values."""
    first = _session()
    second = _session()
...
def test_multiple_sessions_can_hold_the_same_provider_reference() -> None:
    """Conversation state does not impose provider-global continuation uniqueness."""
    ref = _ref("codex", "thread-1")

    first = _session(conversations=(ref,))
    second = _session(conversations=(ref,))

    assert first.conversation_for(InteractionSource("codex")) is ref
    assert second.conversation_for(InteractionSource("codex")) is ref
...
def test_session_turn_releases_after_exception() -> None:
    """Turn exceptions propagate without retaining Conversation coordination."""
    session = _session()
...
def test_session_turn_serializes_same_session_workers() -> None:
    """A second same-Conversation worker enters only after the first exits."""
    session = _session()
...
def test_session_turn_allows_different_sessions_to_proceed_concurrently() -> None:
    """Independent Conversation objects own independent coordination state."""
    first = _session()
    second = _session()
    ...
    async def exercise() -> None:
        ...
        async def hold(session: Conversation, entered: asyncio.Event) -> None:
            async with session.turn():
                entered.set()
                await release.wait()
...
def test_session_turns_do_not_share_locks_for_equal_session_ids() -> None:
    """Separate reconstructed Conversation objects coordinate only locally."""
    first = _session()
    second = _session()
