...
from devtools.agents.conversation import History
from devtools.agents.conversation.history import History as HistoryFromSubmodule
...
def test_history_preserves_insertion_order_not_message_timestamps() -> None:
    """Tuple position, rather than ConversationMessage metadata, determines transcript order."""
    later = _message(
...
def test_history_provides_normal_sequence_length_iteration_and_membership() -> None:
    """Sequence operations retain order and use ConversationMessage value equality."""
...
def test_history_inherits_standard_sequence_conveniences() -> None:
    """Standard Sequence helpers retain ordinary ConversationMessage value semantics."""
    first, second, third, fourth = _messages()
