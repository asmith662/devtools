...
import devtools.codex
from devtools.agents import Agent, ConversationRef
from devtools.codex import CodexAgent, CodexCommandError, CodexError, CodexOutputError
from devtools.codex.agent import _resolve_executable
...
def test_agent_builds_fresh_read_only_jsonl_command_and_maps_turn(
    ...
    agent: Agent = _agent(executor, tmp_path)
    turn = asyncio.run(agent.send(_user_message()))

    assert agent.source == MessageSource("codex")
    assert turn.message.content == "Codex reply"
    assert turn.message.role == MessageRole.ASSISTANT
    assert turn.message.source == agent.source
    assert turn.conversation == ConversationRef(agent.source, "thread-42")
...
def test_agent_rejects_unsuccessful_truncated_or_invalid_results(
    ...
    """Only an untruncated successful command can become an AgentTurn."""
...
def test_default_windows_executable_prefers_the_direct_cmd_launcher(
    ...
    launcher = r"C:\\npm\\codex.cmd"
    monkeypatch.setattr("devtools.codex.agent.sys.platform", "win32")
    monkeypatch.setattr("devtools.codex.agent.shutil.which", lambda _: launcher)
...
def test_default_windows_executable_falls_back_to_command_domain_lookup(
    ...
    monkeypatch.setattr("devtools.codex.agent.sys.platform", "win32")
    monkeypatch.setattr("devtools.codex.agent.shutil.which", lambda _: None)
...
def test_explicit_executable_is_preserved_and_non_windows_keeps_codex(
    ...
    monkeypatch.setattr("devtools.codex.agent.sys.platform", "win32")
    monkeypatch.setattr("devtools.codex.agent.shutil.which", lambda _: "ignored")

    assert _resolve_executable(r"C:\\custom\\codex.exe") == r"C:\\custom\\codex.exe"

    monkeypatch.setattr("devtools.codex.agent.sys.platform", "linux")
    assert _resolve_executable("codex") == "codex"


def test_root_public_api_is_exact() -> None:
    """Only supported adapter and error boundaries are root-exported."""
    assert devtools.codex.__all__ == [
        "CodexAgent",
        "CodexCommandError",
        "CodexError",
        "CodexOutputError",
    ]
    assert CodexError is devtools.codex.CodexError
