# Copyright (c) 2026
"""Tests for Interaction value objects."""
...
def _message(source: MessageSource) -> Message:
    """Create a deterministic interaction response message."""
    return Message(
...
def test_interaction_turn_supports_stateless_and_matching_continued_value_semantics(
) -> None:
    """Turns preserve final messages and optional same-source continuation state."""
    source = MessageSource("codex")
    message = _message(source)
    stateless = InteractionTurn(message)
    reference = ConversationRef(source, "thread-42")
    continued = InteractionTurn(message=message, conversation=reference)
    equivalent = InteractionTurn(message=message, conversation=reference)
...
def test_interaction_turn_accepts_equal_distinct_source_values() -> None:
    ...
    turn = InteractionTurn(message=message, conversation=conversation)

    assert turn.conversation is conversation


def test_interaction_turn_rejects_a_conversation_owned_by_another_source() -> None:
    ...
    with pytest.raises(ValueError, match="must match"):
        InteractionTurn(message=message, conversation=reference)
